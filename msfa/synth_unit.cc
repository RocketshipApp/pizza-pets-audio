...
SynthUnit::SynthUnit(RingBuffer *ring_buffer) {
  ...
  memcpy(patch_data_, msfa_epiano, sizeof(msfa_epiano));
  ProgramChange(0);
  current_note_ = 0;
...
void SynthUnit::SetVolume(float volume) {
    volume_ = volume;
}
...
void SynthUnit::GetSamples(int n_samples, int16_t *buffer) {
  ...
  for (; i < n_samples; i += MSFA_N) {
    AlignedBuf<int32_t, MSFA_N> audiobuf;
    AlignedBuf<int32_t, MSFA_N> audiobuf2;
    for (int j = 0; j < MSFA_N; ++j) {
      audiobuf.get()[j] = 0;
    }
    ...
    for (int j = 0; j < MSFA_N; ++j) {
      ...
      int clip_val = val < -(1 << 24) ? 0x8000 : val >= (1 << 24) ? 0x7fff :
        val >> 9;
      clip_val = (int)((float)clip_val * volume_);
