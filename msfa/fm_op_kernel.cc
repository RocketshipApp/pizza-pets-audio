...
void neon_fm_kernel(const int *in, const int *busin, int *out, int count, int32_t phase0, int32_t freq_arg, int32_t gain1_arg, int32_t dgain_arg) {
...
const int32_t __attribute__ ((aligned(16))) zeros[MSFA_N] = {0};
...
void FmOpKernel::compute(int32_t *output, const int32_t *input,
    ...
    int32_t dgain = (gain2 - gain1 + (MSFA_N >> 1)) >> MSFA_LG_N;
    int32_t gain = gain1;
    int32_t phase = phase0;
    if (hasNeon()) {
#ifdef HAVE_NEON_INTRINSICS
        neon_fm_kernel(input, add ? output : zeros, output, MSFA_N,
                       phase0, freq, gain, dgain);
#endif
    } else {
        if (add) {
            for (int i = 0; i < MSFA_N; i++) {
                gain += dgain;
...
void FmOpKernel::compute_pure(int32_t *output, int32_t phase0, int32_t freq,
    ...
    if (hasNeon()) {
#ifdef HAVE_NEON_INTRINSICS
        neon_fm_kernel(zeros, add ? output : zeros, output, MSFA_N,
                       phase0, freq, gain, dgain);
#endif
    } else {
        if (add) {
...
void FmOpKernel::compute_pure(int32_t *output, int32_t phase0, int32_t freq,
                              int32_t gain1, int32_t gain2, bool add) {
    int32_t dgain = (gain2 - gain1 + (MSFA_N >> 1)) >> MSFA_G_N;
    int32_t gain = gain1;
    int32_t phase = phase0;
#ifdef HIGH_ACCURACY
    int32_t u = Sin::compute10(phase << 6);
